...
public class DateTimeString implements Comparable<DateTimeString> {
  ...
  private static final Pattern PATTERN_WITH_TIME =
      Pattern.compile("[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]");
   ...
   * seconds. */
  private static final Pattern INPUT_PATTERN_WITH_TIME =
      Pattern.compile("[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]"
          + " "
          + "[0-9][0-9]:[0-9][0-9]:[0-9][0-9](\\.[0-9]+)?");

  /** The Unix epoch. */
  public static final DateTimeString EPOCH =
      new DateTimeString(1970, 1, 1, 0, 0, 0);

  final String v;

  /** Creates a TimeString. */
  public DateTimeString(String v) {
    this.v = normalize(v);
  }

  /** Creates a TimestampString for year, month, day, hour, minute, second,
   * millisecond values. */
  public DateTimeString(int year, int month, int day, int h, int m, int s) {
  ...
  public DateTimeString withMillis(int millis) {
  ...
  public DateTimeString withNanos(int nanos) {
  ...
  public DateTimeString withFraction(String fraction) {
    String v = this.v;
    int i = v.indexOf('.');
    if (i >= 0) {
      v = v.substring(0, i);
    ...
    return new DateTimeString(v);
  }

  private static String normalize(String v) {
    checkArgument(
        INPUT_PATTERN.matcher(v).matches()
        || INPUT_PATTERN_WITH_TIME.matcher(v).matches(), v);
    ...
    checkArgument(PATTERN.matcher(v).matches() || PATTERN_WITH_TIME.matcher(v).matches(), v);
    return v;
  }
  ...
  @Override public boolean equals(@Nullable Object o) {
    ...
    return o == this
        || o instanceof DateTimeString
        && ((DateTimeString) o).v.equals(v);
  }
  ...
  @Override public int compareTo(DateTimeString o) {
    return v.compareTo(o.v);
  }

  /** Creates a TimestampString from a Calendar. */
  public static DateTimeString fromCalendarFields(Calendar calendar) {
  ...
  public DateTimeString round(int precision) {
   ...
   * the epoch. */
  @JsonCreator
  public static DateTimeString fromMillisSinceEpoch(
      @JsonProperty("millisSinceEpoch") long millis) {
    return new DateTimeString(DateTimeUtils.unixTimestampToString(millis))
